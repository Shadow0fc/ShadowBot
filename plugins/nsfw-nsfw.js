...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode, isPrems }) => {
	...
	// ------- OTHER ------
  if (global.db.data.chats[m.chat].nsfw == false && m.isGroup) return conn.sendButton(m.chat, '❗ La opción _-NSFW-_ esta desactivada en este grupo.', wm2, null, [['ACTIVAR NSFW', '.enable nsfw']], m)
...
//> Xteam //Not Use Xteam
let xres = 'https://api.xteam.xyz/api/nsfw'
let xapi = '?apikey=xgram'
//--------------------------------
...
let next = '𝙎𝙞𝙜𝙪𝙞𝙚𝙣𝙩𝙚'
let fot = wm
let txtprem = '\`\`\`❗ Los comandos _-NSFW-_ estan divididos en dos categorías;\`\`\`\n *-FREE-* & *-PREMIUM-*\n*❏ Si eres premium tienes acceso a todos los comandos, en cambio si eres usuario -FREE- solo tendras acceso a cierta cantidad de comandos NSFW*'
let p = '❏ 🆓 •'
let f = '❏ 🎖️ •'

let tekk = `\`\`\`❏ 🌌 Aqui tienes!!\`\`\`\n${args[0] ? args[0].capitalize() : false}`
  let teks = `┊ *📮 Use el comando con la categoría que desea.*
┊❏🎆 _*Ejemplo; ${usedPrefix}nsfw neko*_
❏──···––\n\n`
//---------------------------
...
case 'ahegao':
	    ...
	    if (!isPrems) return conn.sendButton(m.chat, txtprem, wm2, [['COMPRAR PREMIUM', '.prembuy'],['Owner', '.owner']], m)
		}
...
case 'anal':
	     ...
	     if (!isPrems) return conn.sendButton(m.chat, txtprem, wm2, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
         }
