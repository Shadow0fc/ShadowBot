...
let exp = `${pickRandom([50000, 60000, 70000, 80000, 90000, 99900, 100000, 130000, 150000, 180000])}` * 1
let exppremium = `${pickRandom([100000, 150000, 1800, 2100, 2500, 2900, 3300, 3600, 4000, 4500])}` * 1
	
let money = `${pickRandom([300, 500, 700, 900, 500, 800, 900, 1100, 1350, 1500])}` * 1
let moneypremium = `${pickRandom([8000, 13000, 16000, 19000, 22000, 25000, 27000, 30000, 33000, 3500])}` * 1
...
let time = user.lastpago + 36000000 //2 Horas 7200000
if (new Date - user.lastpago < 36000000) return await conn.sendButton(m.chat, `𝙔𝘼 𝙍𝙀𝘾𝙇𝘼𝙈𝘼𝙎𝙏𝙀 𝙏𝙐 𝙍𝙀𝙂𝘼𝙇𝙊 🎁\n𝙑𝙐𝙀𝙇𝙑𝙀 𝙀𝙉 *${msToTime(time - new Date())}* 𝙋𝘼𝙍𝘼 𝙑𝙊𝙇𝙑𝙀𝙍 𝘼 𝙍𝙀𝘾𝙇𝘼𝙈𝘼𝙍`, wm, null, [['𝗠 𝗘 𝗡 𝗨 ☘️', '/menu']], fkontak, m)
let texto = ''
