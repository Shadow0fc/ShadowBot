        ...
        for (let name in global.plugins) {
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*❑ ⚠️ 𝗦𝘂𝘀 𝗺𝗼𝗻𝗲𝗱𝗮𝘀 𝘀𝗲 𝗮𝗴𝗼𝘁𝗮𝗿𝗼𝗻, 𝗰𝗼𝗺𝗽𝗿𝗲 𝗺𝗮𝘀 𝘂𝘀𝗮𝗻𝗱𝗼: ${usedPrefix}buy <cantidad>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*❑ ⚠️ 𝗡𝗶𝘃𝗲𝗹 𝗿𝗲𝗾𝘂𝗲𝗿𝗶𝗱𝗼 𝗽𝗮𝗿𝗮 𝘂𝘀𝗮𝗿 𝗲𝘀𝘁𝗲 𝗰𝗼𝗺𝗮𝗻𝗱𝗼: ${plugin.level}\n*𝗧𝘂 𝗻𝗶𝘃𝗲𝗹: ${_user.level}*`, m)
                ...
                try {
                    ...
                    if (m.limit)
                        m.reply(+m.limit + '*_𝗨𝘁𝗶𝗹𝗶𝘇𝗮𝗱𝗮 🪙 (ㆁωㆁ)_*')
                }
                break
            }
        }
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    let apii = await this.getFile(pp)
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*❑ 🗂️ 𝗦𝗶𝗻 𝗱𝗲𝘀𝗰𝗿𝗶𝗽𝗰𝗶𝗼𝗻....*') :
...
export async function callUpdate(callUpdate) {
    ...
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;   𝆬⃝𝆬⃝🌃️ ˖ 𝗦𝗵𝗮𝗱𝗼𝘄𝗕𝗼𝘁-𝗠𝗗 ᘝ ׄ ː;;;\nFN:Shadow\nORG:Shadow-MD 👑\nTITLE:\nitem1.TEL;waid=522431284029:+52 243 128 4029\nitem1.X-ABLabel:   𝆬⃝𝆬⃝🌃️ ˖ 𝗦𝗵𝗮𝗱𝗼𝘄𝗕𝗼𝘁-𝗠𝗗 ᘝ ׄ ː\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:ιαм.⚜️𝐓𝐡𝐞𝐒𝐡𝐚𝐝𝐨𝐰𝐆.ℓι\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '   𝆬⃝𝆬⃝🌃️ ˖ 𝗦𝗵𝗮𝗱𝗼𝘄𝗕𝗼𝘁-𝗠𝗗 ᘝ ׄ☁️໋', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '❑ ⚠️ *_Regístrese para usar esta función  Escribiendo:_*\n\n• */reg nombre.edad*\n\n*_📌Ejemplo_* : */reg Shadow.16*',
        restrict: '❑ ❗ *_¡Esta característica está -deshabilitada-_*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '\n  𝆬⃝𝆬⃝🌃️ ˖ 𝗦𝗵𝗮𝗱𝗼𝘄𝗕𝗼𝘁-𝗠𝗗 ᘝ ׄ ː\n', body: 'ιαм.⚜️𝐓𝐡𝐞𝐒𝐡𝐚𝐝𝐨𝐰𝐆.ℓι', thumbnail: imagen1, sourceUrl: 'https://chat.whatsapp.com/JQMyVsc8U4iEHi7qLFgYNH' }}}}, aa)
